<template>
  <div>
    <b-modal
      v-model="restoreStudent"
      :title="$t('Restore Student')"
      :ok-title="$t('Restore')"
      :cancel-title="$t('cancel')"
      ok-variant="success"
      cancel-variant="primary"
      @close="restoreStudent=false"
      @cancel="restoreStudent=false"
      @ok="ConfirmRestore"
    >
      <b-badge
        class="mt-1"
      ...
      >
        {{ $t('are you sure you want to restore this Student') }}
      </b-badge>
    </b-modal>
    <h3 class="mb-3">
      {{ $t('Students') }}
    ...
    >
      ...
      >
        ...
        <template #cell(options)="data">
          <div class="w-max">
            <a
              v-if="haveAccess('Restore Student')||haveElementAccess('Restore Student Owner',data.item.created_by_id)"
              class="ml-1 text-success"
              @click="RestoreStudent(data.item.id)"
            >
    ...
    >
      <b-row>
        ...
        <b-col
          v-if="haveAccess(['Restore Student','Restore Student Owner'])"
          cols="6"
          md="3"
          class="mb-1"
        >
...
export default {
  ...
  computed: {
    ...
    items() {
      return this.$store.getters['student/TrashedStudents']
    },
    totalRows() {
  ...
  created() {
    this.$store.dispatch('student/GetTrashed')
  },
  methods: {
    RestoreStudent(val) {
      this.restoreStudent = true
      this.student = val
    },
    ConfirmRestore() {
      this.$store.dispatch('student/Restore', this.student)
    },
  },
}
</script>
