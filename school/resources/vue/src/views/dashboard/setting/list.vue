<template>
  <div>
    ...
    <b-row>
      <b-col cols="6">
        <b-button
          v-if="haveAccess('Create Setting')"
          variant="gradient-success"
          @click="$router.push({name:'setting.create'})"
        >
          {{ $t('Create new') }}
        </b-button>
      </b-col>
      ...
      >
        <b-button
          v-if="haveAccess(['Show Setting Trash','Show Setting Trash Owner'])"
          variant="gradient-danger"
          @click="$router.push({name:'setting.trashed'})"
        >
          <font-awesome-icon
            :icon="['fas', 'trash']"
          />
    ...
    >
      ...
      >
        ...
        <template #cell(options)="data">
          <div class="w-max">
            <router-link
              v-if="haveAccess('Update Setting')||haveElementAccess('Update Setting Owner',data.item.created_by_id)"
              class="text-success"
              :to="{name:'setting.edit',params: { setting: data.item.id }}"
            >
              <font-awesome-icon
                :icon="['fas', 'edit']"
              />
            ...
            <a
              v-if="haveAccess('Delete Setting')||haveElementAccess('Delete Setting Owner',data.item.created_by_id)"
...
export default {
  ...
  computed: {
    ...
    items() {
      return this.$store.getters['setting/GetElements']
    },
    totalRows() {
  ...
  created() {
    this.$store.dispatch('setting/GetElements')
  },
  methods: {
    Delete(val) {
    ...
    ConfirmDelete() {
      this.$store.dispatch('setting/Delete', this.element)
    },
  },
}
</script>
