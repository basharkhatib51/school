<template>
  <div>
    <b-modal
      v-model="restoreTeacher"
      :title="$t('Restore Teacher')"
      :ok-title="$t('Restore')"
      :cancel-title="$t('cancel')"
      ok-variant="success"
      cancel-variant="primary"
      @close="restoreTeacher=false"
      @cancel="restoreTeacher=false"
      @ok="ConfirmRestore"
    >
      <b-badge
        class="mt-1"
      ...
      >
        {{ $t('are you sure you want to restore this Teacher') }}
      </b-badge>
    </b-modal>
    <h3 class="mb-3">
      {{ $t('Teachers') }}
    ...
    <b-row>
      <b-col cols="12">
        <b-button
          ...
          @click="$router.push({name:'teacher.list'})"
        >
          <font-awesome-icon
            :icon="['fas', 'arrow-left']"
          />
    ...
    >
      ...
      >
        ...
        <template #cell(options)="data">
          <div class="w-max">
            <a
              v-if="haveAccess('Restore Teacher')||haveElementAccess('Restore Teacher Owner',data.item.created_by_id)"
    ...
    >
      <b-row>
        ...
        <b-col
          v-if="haveAccess(['Restore Teacher','Restore Teacher Owner'])"
          cols="6"
          md="3"
          class="mb-1"
        >
...
export default {
  ...
  computed: {
    ...
    items() {
      return this.$store.getters['teacher/TrashedTeachers']
    },
    totalRows() {
  ...
  created() {
    this.$store.dispatch('teacher/GetTrashed')
  },
  methods: {
    RestoreTeacher(val) {
    ...
    ConfirmRestore() {
      this.$store.dispatch('teacher/Restore', this.teacher)
    },
  },
}
</script>
