<template>
  <div>
    ...
    >
      ...
      >
        {{ $t('are you sure you want to restore this Tag') }}
      </b-badge>
    </b-modal>
    <h3 class="mb-3">
      {{ $t('Tags') }}
    ...
    <b-row>
      <b-col cols="12">
        <b-button
          ...
          @click="$router.push({name:'tag.list'})"
        >
          <font-awesome-icon
            :icon="['fas', 'arrow-left']"
          />
    ...
    >
      ...
      >
        ...
        <template #cell(options)="data">
          <div class="w-max">
            <a
              v-if="haveAccess('Restore Tag')||haveElementAccess('Restore Tag Owner',data.item.created_by_id)"
              class="ml-1 text-success"
              @click="RestoreTag(data.item.id)"
            >
              <font-awesome-icon
...
export default {
  ...
  computed: {
    ...
    items() {
      return this.$store.getters['tag/Trashed']
    },
    totalRows() {
  ...
  created() {
    this.$store.dispatch('tag/GetTrashed')
  },
  methods: {
    RestoreTag(val) {
    ...
    ConfirmRestore() {
      this.$store.dispatch('tag/Restore', this.element)
    },
  },
}
</script>
