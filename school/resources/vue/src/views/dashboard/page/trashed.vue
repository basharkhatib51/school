<template>
  <div>
    ...
    >
      ...
      >
        {{ $t('are you sure you want to restore this Page') }}
      </b-badge>
    </b-modal>
    <h3 class="mb-3">
      {{ $t('Pages') }}
    ...
    >
      ...
      >
        ...
        <template #cell(options)="data">
          <div class="w-max">
            <a
              v-if="haveAccess('Restore Page')||haveElementAccess('Restore Page Owner',data.item.created_by_id)"
              class="ml-1 text-success"
              @click="RestorePage(data.item.id)"
            >
              <font-awesome-icon
...
export default {
  ...
  computed: {
    ...
    items() {
      return this.$store.getters['page/Trashed']
    },
    totalRows() {
  ...
  created() {
    this.$store.dispatch('page/GetTrashed')
  },
  methods: {
    RestorePage(val) {
    ...
    ConfirmRestore() {
      this.$store.dispatch('page/Restore', this.element)
    },
  },
}
</script>
