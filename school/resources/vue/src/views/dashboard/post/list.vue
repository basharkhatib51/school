<template>
  <div>
    ...
    <b-row>
      ...
      >
        <b-button
          v-if="haveAccess(['Show Post Trash','Show Post Trash Owner'])"
          variant="gradient-danger"
          @click="$router.push({name:'post.trashed'})"
        >
          <font-awesome-icon
            :icon="['fas', 'trash']"
          />
    ...
    >
      ...
      >
        ...
        <template #cell(options)="data">
          <div class="w-max">
            <router-link
              v-if="haveAccess('Update Post')||haveElementAccess('Update Post Owner',data.item.created_by_id)"
              class="text-success"
              :to="{name:'post.edit',params: { post: data.item.id }}"
            >
              <font-awesome-icon
                :icon="['fas', 'edit']"
              />
            ...
            <a
              v-if="haveAccess('Delete Post')||haveElementAccess('Delete Post Owner',data.item.created_by_id)"
...
export default {
  ...
  computed: {
    ...
    items() {
      return this.$store.getters['post/GetElements']
    },
    totalRows() {
  ...
  methods: {
    ...
    ConfirmDelete() {
      this.$store.dispatch('post/Delete', this.element)
    },
  },
}
</script>
