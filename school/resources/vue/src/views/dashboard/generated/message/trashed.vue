    ...
    <b-row>
      <b-col cols="12">
        <b-button
          ...
          @click="$router.push({name:'message.list'})"
        >
          <font-awesome-icon
            :icon="['fas', 'arrow-left']"
          />
    ...
    >
      ...
      >
        ...
        <template #cell(options)="data">
          <div class="w-max">
            <a
              v-if="haveAccess('Restore Message')||haveElementAccess('Restore Message Owner',data.item.created_by_id)"
...
export default {
  ...
  computed: {
    items() {
      return this.$store.getters['message/Trashed']
    },
    totalRows() {
  ...
  created() {
    this.$store.dispatch('message/GetTrashed')
  },
  methods: {
    RestoreMessage(val) {
    ...
    ConfirmRestore() {
      this.$store.dispatch('message/Restore', this.element)
    },
  },
}
</script>
