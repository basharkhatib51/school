<template>
  <div>
    ...
    >
      ...
      >
        {{ $t('are you sure you want to restore this Subject') }}
      </b-badge>
    </b-modal>
    <h3 class="mb-3">
      {{ $t('Subjects') }}
    ...
    <b-row>
      <b-col cols="12">
        <b-button
          ...
          @click="$router.push({name:'subject.list'})"
        >
          <font-awesome-icon
            :icon="['fas', 'arrow-left']"
          />
    ...
    >
      ...
      >
        ...
        <template #cell(subjectregistrations)="data">
          <b-badge
            pill
            variant="light-primary"
          >
            {{ data.item['subjectregistrations_count'] }} SubjectRegistrations
          </b-badge>
        </template>
        ...
        <template #cell(options)="data">
          <div class="w-max">
            <a
              v-if="haveAccess('Restore Subject')||haveElementAccess('Restore Subject Owner',data.item.created_by_id)"
              class="ml-1 text-success"
              @click="RestoreSubject(data.item.id)"
            >
              <font-awesome-icon
...
export default {
  ...
  computed: {
    items() {
      return this.$store.getters['subject/Trashed']
    },
    totalRows() {
  ...
  created() {
    this.$store.dispatch('subject/GetTrashed')
  },
  methods: {
    RestoreSubject(val) {
    ...
    ConfirmRestore() {
      this.$store.dispatch('subject/Restore', this.element)
    },
  },
}
</script>
