<template>
  <div>
    ...
    >
      ...
      >
        {{ $t('are you sure you want to restore this Classroom') }}
      </b-badge>
    </b-modal>
    <h3 class="mb-3">
      {{ $t('Classrooms') }}
    ...
    <b-row>
      <b-col cols="12">
        <b-button
          ...
          @click="$router.push({name:'classroom.list'})"
        >
          <font-awesome-icon
            :icon="['fas', 'arrow-left']"
          />
    ...
    >
      ...
      >
        ...
        <template #cell(options)="data">
          <div class="w-max">
            <a
              v-if="haveAccess('Restore Classroom')||haveElementAccess('Restore Classroom Owner',data.item.created_by_id)"
              class="ml-1 text-success"
              @click="RestoreClassroom(data.item.id)"
            >
              <font-awesome-icon
...
export default {
  ...
  computed: {
    items() {
      return this.$store.getters['classroom/Trashed']
    },
    totalRows() {
  ...
  created() {
    this.$store.dispatch('classroom/GetTrashed')
  },
  methods: {
    RestoreClassroom(val) {
      this.restoreElement = true
      this.element = val
    },
    ConfirmRestore() {
      this.$store.dispatch('classroom/Restore', this.element)
    },
  },
}
</script>
