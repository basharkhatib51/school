<template>
  <div>
    ...
    <b-row>
      <b-col cols="6">
        <b-button
          v-if="haveAccess('Create Classroom')"
          variant="gradient-success"
          @click="$router.push({name:'classroom.create'})"
        >
          {{ $t('Create new') }}
        </b-button>
      </b-col>
      ...
      >
        <b-button
          v-if="haveAccess(['Show Classroom Trash','Show Classroom Trash Owner'])"
          variant="gradient-danger"
          @click="$router.push({name:'classroom.trashed'})"
        >
          <font-awesome-icon
            :icon="['fas', 'trash']"
          />
...
export default {
  ...
  methods: {
    ...
    ConfirmDelete() {
      this.$store.dispatch('classroom/Delete', this.element)
    },
  },
}
</script>
