<template>
  <div>
    ...
    >
      ...
      >
        {{ $t('are you sure you want to restore this Fee') }}
      </b-badge>
    </b-modal>
    <h3 class="mb-3">
      {{ $t('Fees') }}
    ...
    <b-row>
      <b-col cols="12">
        <b-button
          ...
          @click="$router.push({name:'fee.list'})"
        >
          <font-awesome-icon
            :icon="['fas', 'arrow-left']"
          />
    ...
    >
      ...
      >
        ...
        <template #cell(options)="data">
          <div class="w-max">
            <a
              v-if="haveAccess('Restore Fee')||haveElementAccess('Restore Fee Owner',data.item.created_by_id)"
              class="ml-1 text-success"
              @click="RestoreFee(data.item.id)"
            >
              <font-awesome-icon
...
export default {
  ...
  computed: {
    items() {
      return this.$store.getters['fee/Trashed']
    },
    totalRows() {
  ...
  created() {
    this.$store.dispatch('fee/GetTrashed')
  },
  methods: {
    RestoreFee(val) {
    ...
    ConfirmRestore() {
      this.$store.dispatch('fee/Restore', this.element)
    },
  },
}
</script>
