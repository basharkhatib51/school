<template>
  <div>
    ...
    >
      ...
      >
        {{ $t('are you sure you want to delete this ClassLevel') }}
      </b-badge>
    </b-modal>
    <h3 class="mb-3">
      {{ $t('ClassLevels') }}
    ...
    <b-row>
      <b-col cols="6">
        <b-button
          v-if="haveAccess('Create ClassLevel')"
          variant="gradient-success"
          @click="$router.push({name:'class_level.create'})"
        >
          {{ $t('Create new') }}
        </b-button>
      </b-col>
      ...
      >
        <b-button
          v-if="haveAccess(['Show ClassLevel Trash','Show ClassLevel Trash Owner'])"
          variant="gradient-danger"
          @click="$router.push({name:'class_level.trashed'})"
        >
          <font-awesome-icon
            :icon="['fas', 'trash']"
          />
      ...
      >
        ...
        <template #cell(fees)="data">
          <b-badge
            ...
            variant="light-primary"
          >
            {{ data.item.fees_value ? data.item.fees_value : $t('undefined') }}
          </b-badge>
        </template>
        ...
        <template #cell(options)="data">
          <div class="w-max">
            <router-link
              v-if="haveAccess('Update ClassLevel')||haveElementAccess('Update ClassLevel Owner',data.item.created_by_id)"
              class="text-success"
              :to="{name:'class_level.edit',params: { class_level: data.item.id }}"
            >
              <font-awesome-icon
                :icon="['fas', 'eye']"
              />
            ...
            <a
              v-if="haveAccess('Delete ClassLevel')||haveElementAccess('Delete ClassLevel Owner',data.item.created_by_id)"
...
export default {
  ...
  computed: {
    fields() {
      return [
        ...
        { key: 'subjects', label: this.$t('subjects') },
        { key: 'classrooms', label: this.$t('classrooms') },
        { key: 'fees', label: this.$t('fees') },
  ...
  methods: {
    ...
    ConfirmDelete() {
      this.$store.dispatch('classLevel/Delete', this.element)
    },
  },
}
</script>
