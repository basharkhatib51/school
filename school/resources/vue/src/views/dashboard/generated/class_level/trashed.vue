<template>
  <div>
    ...
    >
      ...
      >
        {{ $t('are you sure you want to restore this ClassLevel') }}
      </b-badge>
    </b-modal>
    <h3 class="mb-3">
      {{ $t('ClassLevels') }}
    ...
    <b-row>
      <b-col cols="12">
        <b-button
          ...
          @click="$router.push({name:'class_level.list'})"
        >
          <font-awesome-icon
            :icon="['fas', 'arrow-left']"
          />
    ...
    >
      ...
      >
        ...
        </template>        <template #cell(subjects)="data">
          <b-badge
            pill
            variant="light-primary"
          >
            {{ data.item['subjects_count'] }} Subjects
          </b-badge>
        </template>
        <template #cell(classrooms)="data">
          <b-badge
            ...
            variant="light-primary"
          >
            {{ data.item['classrooms_count'] }} Classrooms
          </b-badge>
        </template>
        ...
        <template #cell(notifications)="data">
          <b-badge
            ...
            variant="light-primary"
          >
            {{ data.item['notifications_count'] }} Notifications
          </b-badge>
        </template>
        ...
        <template #cell(options)="data">
          <div class="w-max">
            <a
              v-if="haveAccess('Restore ClassLevel')||haveElementAccess('Restore ClassLevel Owner',data.item.created_by_id)"
              class="ml-1 text-success"
              @click="RestoreClassLevel(data.item.id)"
            >
              <font-awesome-icon
...
export default {
  ...
  computed: {
    ...
    items() {
      return this.$store.getters['classLevel/Trashed']
    },
    totalRows() {
  ...
  created() {
    this.$store.dispatch('classLevel/GetTrashed')
  },
  methods: {
    RestoreClassLevel(val) {
      this.restoreElement = true
      this.element = val
    },
    ConfirmRestore() {
      this.$store.dispatch('classLevel/Restore', this.element)
    },
  },
}
</script>
