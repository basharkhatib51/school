<template>
  <div>
    <b-modal
      ...
      :title="$t('Restore StudentRegistration')"
      :ok-title="$t('Restore')"
    ...
    >
      ...
      >
        {{ $t('are you sure you want to restore this StudentRegistration') }}
      </b-badge>
    </b-modal>
    <h3 class="mb-3">
    ...
    <b-row>
      <b-col cols="12">
        <b-button
          ...
          @click="$router.push({name:'student_registration.list'})"
        >
          <font-awesome-icon
            :icon="['fas', 'arrow-left']"
          />
    ...
    >
      ...
      >
        ...
        <template #cell(options)="data">
          <div class="w-max">
            <a
              v-if="haveAccess('Restore StudentRegistration')||haveElementAccess('Restore StudentRegistration Owner',data.item.created_by_id)"
              class="ml-1 text-success"
              @click="RestoreStudentRegistration(data.item.id)"
            >
              <font-awesome-icon
...
export default {
  ...
  computed: {
    items() {
      return this.$store.getters['studentRegistration/Trashed']
    },
    totalRows() {
  ...
  created() {
    this.$store.dispatch('studentRegistration/GetTrashed')
  },
  methods: {
    RestoreStudentRegistration(val) {
      this.restoreElement = true
      this.element = val
    },
    ConfirmRestore() {
      this.$store.dispatch('studentRegistration/Restore', this.element)
    },
  },
}
</script>
