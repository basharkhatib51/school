<template>
  <div>
    <b-modal
      ...
      :title="$t('Delete StudentRegistration')"
      :ok-title="$t('Delete')"
    ...
    >
      ...
      >
        {{ $t('are you sure you want to delete this StudentRegistration') }}
      </b-badge>
    </b-modal>
    <h3 class="mb-3">
      {{ $t('StudentRegistrations') }}
    </h3>
    <b-row>
      <b-col cols="6">
        <b-button
          v-if="haveAccess('Create StudentRegistration')"
          variant="gradient-success"
          @click="$router.push({name:'student_registration.create'})"
        >
          {{ $t('Create new') }}
        </b-button>
      </b-col>
      ...
      >
        <b-button
          v-if="haveAccess(['Show StudentRegistration Trash','Show StudentRegistration Trash Owner'])"
          variant="gradient-danger"
          @click="$router.push({name:'student_registration.trashed'})"
        >
          <font-awesome-icon
            :icon="['fas', 'trash']"
          />
    ...
    >
      ...
      >
        ...
        </template>        <template #cell(student)="data">
          <b-badge
            pill
        ...
        <template #cell(payments)="data">
          <b-badge
            ...
            variant="light-primary"
          >
            {{ data.item['payments_count'] }} Payments
          </b-badge>
        </template>
        ...
        <template #cell(options)="data">
          <div class="w-max">
            <router-link
              v-if="haveAccess('Update StudentRegistration')||haveElementAccess('Update StudentRegistration Owner',data.item.created_by_id)"
              class="text-success"
              :to="{name:'student_registration.edit',params: { student_registration: data.item.id }}"
            >
              <font-awesome-icon
                :icon="['fas', 'edit']"
              />
            ...
            <a
              v-if="haveAccess('Delete StudentRegistration')||haveElementAccess('Delete StudentRegistration Owner',data.item.created_by_id)"
...
export default {
  ...
  computed: {
    items() {
      return this.$store.getters['studentRegistration/GetElements']
    },
    totalRows() {
  ...
  methods: {
    ...
    ConfirmDelete() {
      this.$store.dispatch('studentRegistration/Delete', this.element)
    },
  },
}
</script>
