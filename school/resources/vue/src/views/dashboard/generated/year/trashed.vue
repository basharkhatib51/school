<template>
  <div>
    ...
    >
      ...
      >
        {{ $t('are you sure you want to restore this Year') }}
      </b-badge>
    </b-modal>
    <h3 class="mb-3">
      {{ $t('Years') }}
    ...
    <b-row>
      <b-col cols="12">
        <b-button
          ...
          @click="$router.push({name:'year.list'})"
        >
          <font-awesome-icon
            :icon="['fas', 'arrow-left']"
          />
    ...
    >
      ...
      >
        ...
        <template #cell(options)="data">
          <div class="w-max">
            <a
              v-if="haveAccess('Restore Year')||haveElementAccess('Restore Year Owner',data.item.created_by_id)"
              class="ml-1 text-success"
              @click="RestoreYear(data.item.id)"
            >
              <font-awesome-icon
...
export default {
  ...
  computed: {
    items() {
      return this.$store.getters['year/Trashed']
    },
    totalRows() {
  ...
  created() {
    this.$store.dispatch('year/GetTrashed')
  },
  methods: {
    RestoreYear(val) {
    ...
    ConfirmRestore() {
      this.$store.dispatch('year/Restore', this.element)
    },
  },
}
</script>
