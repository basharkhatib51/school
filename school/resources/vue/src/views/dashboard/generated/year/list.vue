<template>
  <div>
    ...
    <b-row>
      <b-col cols="6">
        <b-button
          v-if="haveAccess('Create Year')"
          variant="gradient-success"
          @click="$router.push({name:'year.create'})"
        >
          {{ $t('Create new') }}
        </b-button>
      </b-col>
      ...
      >
        <b-button
          v-if="haveAccess(['Show Year Trash','Show Year Trash Owner'])"
          variant="gradient-danger"
          @click="$router.push({name:'year.trashed'})"
        >
          <font-awesome-icon
            :icon="['fas', 'trash']"
          />
    ...
    >
      ...
      >
        ...
        </template>        <template #cell(subject_registrations)="data">
          <b-badge
            pill
            variant="light-primary"
          >
        ...
        <template #cell(student_registrations)="data">
          <b-badge
            pill
            variant="light-primary"
          >
            {{ data.item['student_registrations_count'] }} StudentRegistrations
          </b-badge>
        </template>
        ...
        <template #cell(options)="data">
          <div class="w-max">
            <router-link
              v-if="haveAccess('Update Year')||haveElementAccess('Update Year Owner',data.item.created_by_id)"
              class="text-success"
              :to="{name:'year.edit',params: { year: data.item.id }}"
            >
              <font-awesome-icon
                :icon="['fas', 'edit']"
              />
            ...
            <a
              v-if="haveAccess('Delete Year')||haveElementAccess('Delete Year Owner',data.item.created_by_id)"
...
export default {
  ...
  computed: {
    fields() {
      return [
        ...
        { key: 'student_registrations', label: this.$t('student_registrations') },
  ...
  methods: {
    ...
    ConfirmDelete() {
      this.$store.dispatch('year/Delete', this.element)
    },
  },
}
</script>
