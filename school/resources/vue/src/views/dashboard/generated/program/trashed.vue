<template>
  <div>
    ...
    >
      ...
      >
        {{ $t('are you sure you want to restore this Program') }}
      </b-badge>
    </b-modal>
    <h3 class="mb-3">
      {{ $t('Programs') }}
    ...
    <b-row>
      <b-col cols="12">
        <b-button
          ...
          @click="$router.push({name:'program.list'})"
        >
          <font-awesome-icon
            :icon="['fas', 'arrow-left']"
          />
    ...
    >
      ...
      >
        ...
        </template>        <template #cell(start_at)="data">
          <b-badge
            pill
            variant="light-primary"
          >
        ...
        <template #cell(subject_registration)="data">
          <b-badge
            pill
            :variant="data.item.subject_registration_data?'light-primary':'light-danger'"
          >
            {{ data.item.subject_registration_data?data.item.subject_registration_data.name:$t('Not related yet') }}
          </b-badge>
        </template>
        ...
        <template #cell(options)="data">
          <div class="w-max">
            <a
              v-if="haveAccess('Restore Program')||haveElementAccess('Restore Program Owner',data.item.created_by_id)"
              class="ml-1 text-success"
              @click="RestoreProgram(data.item.id)"
            >
              <font-awesome-icon
...
export default {
  ...
  computed: {
    items() {
      return this.$store.getters['program/Trashed']
    },
    totalRows() {
  ...
  created() {
    this.$store.dispatch('program/GetTrashed')
  },
  methods: {
    RestoreProgram(val) {
    ...
    ConfirmRestore() {
      this.$store.dispatch('program/Restore', this.element)
    },
  },
}
</script>
