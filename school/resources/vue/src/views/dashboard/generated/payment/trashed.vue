<template>
  <div>
    ...
    >
      ...
      >
        {{ $t('are you sure you want to restore this Payment') }}
      </b-badge>
    </b-modal>
    <h3 class="mb-3">
      {{ $t('Payments') }}
    ...
    >
      ...
      >
        ...
        <template #cell(options)="data">
          <div class="w-max">
            <a
              v-if="haveAccess('Restore Payment')||haveElementAccess('Restore Payment Owner',data.item.created_by_id)"
...
export default {
  ...
  computed: {
    items() {
      return this.$store.getters['payment/Trashed']
    },
    totalRows() {
  ...
  created() {
    this.$store.dispatch('payment/GetTrashed')
  },
  methods: {
    RestorePayment(val) {
    ...
    ConfirmRestore() {
      this.$store.dispatch('payment/Restore', this.element)
    },
  },
}
</script>
